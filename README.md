...
**測定できる範囲では「UX要件＋見た目の禁止」の併用が最良。** ただし条件付き。

1. **見た目の禁止リストだけを使うのは、使わないより悪い。** 題材2では対照群がUX全項目を
   クリアしていたのに対し、見た目のみの条件は違反4件。**下位互換であり単独で使う理由がない。**
2. **ただし「良いデザインだ」とは測っていない。** 測ったのはアクセシビリティとAI感の構造指標だけ。
   むしろ制約は作り込みも抑制した——題材1の対照群は本文3,664字・SVGイラスト9点を自発的に
   作ったが、併用条件は432字・SVG1点。「44KB→6KB」の差分の中身はイラストと本文だった。
